...
if __name__ == "__main__":
    ...
    def sample(i, type, block_size = None, top_k = None, remasking_strategy = None):
        if model_base == "dream":
            ...
            script_name = "sdar_rl_rollout_kernel.py"
        elif model_base == "trado":
    ...
    def reward(i, type, is_code_task, block_size = None, top_k = None, remasking_strategy = None):
        if is_code_task:
            script_name = "rl_code_reward_kernel.py"
        else:
    ...
    if config.dataset.data_type == "code" or config.dataset.data_type == "kernel_infilling":
        is_code_task = True
    else:
        is_code_task = False
    ...
    while i <= config.experiment.total_step:
        ...
        if i % config.experiment.eval_every == 0:
            if model_base in ["sdar", "trado"]:
                ...
                remasking_strategy = config.evaluation.remasking_strategy
